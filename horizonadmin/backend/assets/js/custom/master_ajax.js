    ...
    $.ajax({
    	...
    	success: function(response, textStatus, xhr)
        {
            try {
                s_result = "{";
                s_result += '"res":true,';
                s_result += '"msg":"'+textStatus.toString()+ '"';
                s_result += "}";
                output(s_result);
            }
            catch(err) {
        ...
        {
            try {
                if(xhr.status == "404" || xhr.responseText.trim() == "")
                {
                    err_msg = "An error occured: Requested url was not found. or internet connection may be not available.";
                    e_result = "{";
                    e_result += '"res":false,';
                    e_result += '"msg":"'+ err_msg + '",';
