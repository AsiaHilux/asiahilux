<div class="row">
    <div class="col-md-12">
        <div class="card card-underline">
            <div class="card-head">
                <header><h4>View  Car Manufacturer Names</h4></header>
            </div>
            <div class="card-body">
                <div class="row">
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Delete  Car Manufacturer Name</h4>
                </div>
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <input type="hidden" id="vm_id_del" value="" class="form-control"
                               placeholder="vm_id_del" required>
                    </div>
...
<div class="modal fade" id="myModal-files" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
        ...
        var url = "<?= base_url() . 'index.php/'; ?>car_manufacturer/select";

        $("#search").keyup(function () {
            var value = this.value.toLowerCase().trim();
            var url = "<?= base_url() . 'index.php/'; ?>car_manufacturer/select?search_term="+value;

            loadCarManufacturers(url);
        });


        loadCarManufacturers(url);

        function loadCarManufacturers(url) {
            $("#tblbody").empty();
                    ...
                    if (i == "deptype") {
                        ...
                        if (field.length == 0) {
                            ...
                            for (j = 0; j < field.length; j++) {
                                ...
                                rowtags += '<tr><td>' + sn + '</td><td>' + field[j].vm_name + '</td><td style="text-align:center;"><span class="' + status + '"></span></td><td style="text-align:center;"><button id="btn_upload_pic" data="' + field[j].vm_id + '" class="btn btn-primary btn-responsive glyphicon glyphicon-upload" data-toggle="tooltip" data-placement="bottom" data-trigger="hover" data-original-title="Upload Car Manufacturer Image"></button>&nbsp;&nbsp;<button name="btn_edit" data="' + field[j].vm_id + '" class="btn btn-primary glyphicon btn-responsive glyphicon-edit margin-2px"></button>&nbsp;&nbsp;<button name="btn_delete" data="' + field[j].vm_id + '" class="btn btn-danger btn-responsive glyphicon glyphicon-remove"></button></td></tr>';
                                sn += 1;
                            }
        ...
        $('#example3').on('click', 'button[name=btn_edit]', function () {
            ...
            $("#page_content").load
            ("<?= base_url() . 'index.php/'; ?>car_manufacturer/load_edit?id=" + id, function (responseTxt, statusTxt, xhr) {
        ...
        $("#btn_delete").click(function () {
            if ($('#flag_del').val() == "Delete") {
                ...
                url = "<?= base_url() . 'index.php/'; ?>car_manufacturer/delete";
...
$("#btn_upload").click(function(){
    ...
    var upload_url = "<?= base_url() . 'index.php/'; ?>car_manufacturer/upload_attachment_for_featured_Image";
    
    var file_to_upload = input_file[0].files[0]; 
   
    var server_response = [];
    var url = "<?= base_url() . 'index.php/'; ?>car_manufacturer/validate_upload_attanchment?file_name="+file_to_upload.name+"&file_size="+file_to_upload.size;
    $.getJSON(url, function(result){
        ////////////////////////////////////////////////////
