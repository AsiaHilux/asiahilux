...
{
    private $limit = 250;
    private $this_page_id = "country_pg_id";
    ...
    public function index()
    {
        ...
        if ($this->check_permission("read", true)) {
            ...
            $this->load->view('country', $user_data);
        } else {
            $user_data['arr_user'] = $this->get_user_data($this->session->userdata('user_id'));
            $user_data['arr_current_school'] = $this->get_corrent_school($this->session->userdata('horizon_id'));
    ...
    public function load_edit()
    {
        if ($this->check_permission("write", false)) {
            ...
            if (count($result) > 0) {
                ...
                $this->load->view('country_form', $data);
            } else {
    ...
    public function create()
    {
        if ($this->check_permission("write", false)) {
			...
			$config['upload_path'] = realpath("../../assets/country_assets");
			...
			if ($_FILES['banner_1']['name']) {
				$file_name_wo_ext = pathinfo($_FILES['banner_1']['name'], PATHINFO_FILENAME);
				...
				$data['banner_1'] = $filename;
			}
			// banner_4 img upload
			if ($_FILES['banner_4']['name']) {
				$file_name_wo_ext = pathinfo($_FILES['banner_4']['name'], PATHINFO_FILENAME);
				$file_ext_change = $file_name_wo_ext . ".png";
				$config['file_name'] = $file_ext_change;
				$banner_4 = 'banner_4';
				if (!$this->upload->do_upload($banner_4)) {
					print_r($this->upload->display_errors());
				}
				$arr_upload_info = $this->upload->data();
				$filename = $arr_upload_info['file_name'];
				$data['banner_4'] = $filename;
			}
			$config['upload_path'] = realpath("../../assets/images/");
			// flag img upload
			if ($_FILES['flag_img']['name']) {
				$file_name_wo_ext = pathinfo($_FILES['flag_img']['name'], PATHINFO_FILENAME);
				$file_ext_change = $file_name_wo_ext . ".png";
				$config['file_name'] = $file_ext_change;
				$flag_img = 'flag_img';
				...
				$arr_upload_info = $this->upload->data();
				$filename = $arr_upload_info['file_name'];
				$data['stored_file_name'] = $filename;
			}

            $horizon_id = $this->session->userdata('horizon_id');
            $country_name = str_replace('"', "", $this->input->post('country_name'));
            $countries_description = str_replace('"', "", $this->input->post('countries_description'));
            ...
            $data['countries_description'] = $countries_description;
            $data['active'] = $active;
            $data['Title'] = $Title;
            $data['Keywords'] = $Keywords;
            $data['Description'] = $Description;
            $data['shipping'] = str_replace('"', "", $this->input->post('shipping'));
            $data['right_hand_drive'] = str_replace('"', "", $this->input->post('right_hand_drive'));
            $data['year_restriction'] = str_replace('"', "", $this->input->post('year_restriction'));
            $data['inspection'] = str_replace('"', "", $this->input->post('inspection'));
            $data['doc'] = json_encode($this->input->post('doc'));
            

            $res = $this->mdl_dal->insert_data("si_countries", $data);
    ...
    public function update()
    {
        if ($this->check_permission("write", false)) {
			...
			$this->load->library('upload', $config,'countryupload');
			$this->countryupload->initialize($config);
			// flag img upload
			if ($_FILES['flag_img']['name']) {
				$file_name_wo_ext = pathinfo($_FILES['flag_img']['name'], PATHINFO_FILENAME);
				$file_ext_change = $file_name_wo_ext . ".png";
				$config2['file_name'] = $file_ext_change;
				$flag_img = 'flag_img';
				if (!$this->countryupload->do_upload($flag_img)) {
					print_r($this->upload->display_errors());
				}
				$arr_upload_info = $this->countryupload->data();
				$filename = $arr_upload_info['file_name'];
				$setdata['stored_file_name'] = $filename;
			}
			
            $setdata['country_name'] = str_replace('"', "", $this->input->post('country_name'));
            $setdata['countries_description'] = str_replace('"', "", $this->input->post('countries_description'));
            ...
            $setdata['shipping'] = str_replace('"', "", $this->input->post('shipping'));
            $setdata['right_hand_drive'] = str_replace('"', "", $this->input->post('right_hand_drive'));
            $setdata['year_restriction'] = str_replace('"', "", $this->input->post('year_restriction'));
            $setdata['inspection'] = str_replace('"', "", $this->input->post('inspection'));
            $setdata['doc'] = json_encode($this->input->post('doc'));
            ...
            $wheredata['country_id'] = str_replace('"', "", $this->input->post('country_id'));
            $res = $this->mdl_dal->update_data('si_countries', $setdata, $wheredata);
    ...
    public function delete()
    {
        if ($this->check_permission("delete", false)) {
            ...
            //check if this bdy type is used in Car details.
            $sql = "SELECT * FROM hv_car_details
            WHERE Bodytype = ? 
            AND horizon_id = ?";
            $params = array('Bodytype' => $country_id, 'horizon_id' => $horizon_id);

            $records = $this->mdl_dal->select_data2($sql, $params);

            if (count($records) < 1) {
                $wheredata['country_id'] = $country_id;
                $wheredata['horizon_id'] = $horizon_id;

                $res = $this->mdl_dal->delete_data('si_countries', $wheredata);
                ...
                sleep(1);
                echo "This record can't be deleted, its related with other entry in Car Details.";
                http_response_code(500);
            }
    ...
    function select()
    {
        if ($this->check_permission("read", false)) {
            ...
            {
                $sql = "SELECT * FROM si_countries ORDER BY country_name ASC";
            ...
            $config['base_url'] = base_url() . 'index.php/country/select';
    ...
    function select_active()
    {
        if (!$this->check_authentication()) {
            ...
            ///
            $sql = "SELECT * FROM si_countries WHERE active = 1 AND si_countries.horizon_id = " . $this->session->userdata('horizon_id');
    ...
    function select_by_id($id)
    {
        ...
        exit();
              ...
              $sql = "SELECT * FROM si_countries WHERE country_id= " . $id .
    ...
    public function upload_attachment_for_featured_Image()
    {
        ...
        if ($this->upload->do_upload($file)) 
        {
            ...
            $country_id = str_replace('"', "", $this->input->post('car_featured_id_for_upload'));
