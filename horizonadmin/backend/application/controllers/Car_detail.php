...
class Car_detail extends CI_Controller
{
	...
	public function add_new_car()
	{
		if ($this->check_permission("write", false)) {
			...
			$user_data['acc_no'] = $acc_no;
			$user_data['arr_user'] = $this->get_user_data($this->session->userdata('user_id'));
			$user_data['arr_current_school'] = $this->get_corrent_school($this->session->userdata('horizon_id'));
			$user_data['countries'] = $this->mdl_dal->select_data("SELECT * FROM si_countries WHERE si_countries.horizon_id = " . $this->session->userdata('horizon_id'));
			$user_data['bodytypes'] = $this->mdl_dal->select_data("SELECT * FROM hv_car_bodytype WHERE hv_car_bodytype.active = 1 AND hv_car_bodytype.horizon_id = " . $this->session->userdata('horizon_id'));
			$user_data['action'] = 'create';
			$this->load->view('add_new_car', $user_data);
		} else {
			http_response_code(403);
		}
	}
	...
	public function update_car($car_id)
	{
		if ($this->check_permission("write", false)) {
			$car_detail = $this->db
				->select('hv_car_details.*')
				->where('hv_car_details.car_d_id', $car_id)
				->get('hv_car_details')
				->result();

			$user_data['country'] = $this->db
				->select('country_detail_id')
				->where('si_countries_details.car_d_id', $car_id)
				->get('si_countries_details')
				->result();

			$user_data['body'] = $this->db
				->select('bodytype_id')
				->where('hv_car_bodytype_details.car_d_id', $car_id)
				->get('hv_car_bodytype_details')
				->result();
			// echo "<pre>";print_r($country);die;
			$user_data['car_d_id'] = $car_detail[0]->car_d_id;
			$user_data['acc_no'] = $car_detail[0]->acc_no;
			$user_data['car_detail'] = $car_detail[0];
			$user_data['arr_user'] = $this->get_user_data($this->session->userdata('user_id'));
			$user_data['arr_current_school'] = $this->get_corrent_school($this->session->userdata('horizon_id'));
			$user_data['countries'] = $this->mdl_dal->select_data("SELECT * FROM si_countries WHERE si_countries.horizon_id = " . $this->session->userdata('horizon_id'));
			$user_data['bodytypes'] = $this->mdl_dal->select_data("SELECT * FROM hv_car_bodytype WHERE hv_car_bodytype.active = 1 AND hv_car_bodytype.horizon_id = " . $this->session->userdata('horizon_id'));
	...
	public function create2()
	{
		if ($this->check_permission("write", false)) {
			...
			$horizon_id = $this->session->userdata('horizon_id');
			$car_d_id = str_replace('"', "", $this->input->post('set_data')['car_d_id']);
			$_POST['set_data']['horizon_id'] = $horizon_id;

			$query['query_type'] = "insert";
			$query['table_name'] = "hv_car_details";
			$query['query_data'] = $this->input->post('set_data');

			$this->mdl_dal3->send_query($query);

			$Bodytype = $this->input->post('bodytype');

			if (is_array($Bodytype)) {
				foreach ($Bodytype as $by_data) {
					$data1 = array();
					$query = array();

					$data1['car_d_id'] = $car_d_id;
					$data1['bodytype_id'] = $by_data;
					$data1['horizon_id'] = $horizon_id;
			...
			if (is_array($countryid)) {
				foreach ($countryid as $c_data) {
					...
					$data2['car_d_id'] = $car_d_id;
					$data2['country_detail_id'] = $c_data;
					$data2['horizon_id'] = $horizon_id;
			...
			redirect(base_url('index.php/car_detail'));
		} else {
			http_response_code(403);
		}
	}
	...
	public function update2()
	{
		if ($this->check_permission("write", false)) {
			...
			$this->db->where('car_d_id', $car_d_id);
			$result =  $this->db->update('hv_car_details', $this->input->post('set_data'));

			$this->db->where('car_d_id', $car_d_id);
			$result = $this->db->delete('hv_car_bodytype_details');
			if (is_array($Bodytype)) {
				foreach ($Bodytype as $by_data) {
					$data1 = array();
					$query = array();
					...
					$result = $this->db->insert('hv_car_bodytype_details', $data1);
				}
			}

			$this->db->where('car_d_id', $car_d_id);
			$result = $this->db->delete('si_countries_details');
			if (is_array($countryid)) {
				foreach ($countryid as $c_data) {
					...
					$result = $this->db->insert('si_countries_details', $data2);
				}
			}
	...
	public function delete()
	{
		if ($this->check_permission("delete", false)) {
			...
			$this->db->where('car_d_id', $car_d_id);
			$this->db->where('horizon_id', $horizon_id);
			$res = $this->db->update('hv_car_details', ['is_trash' => 1]);
	...
	public function activate_deactivate_car()
	{
		$id = $this->input->post('car_d_id');
		$action = $this->input->post('action');
		$this->db->where('car_d_id', $id)->update('hv_car_details', ['is_active' => $action]);
		redirect($this->input->request_headers()['Referer']);
	}
		...
		if ($this->check_permission("read", false)) {
			...
			if ($search_term == null || $search_term == "" || $search_term == "undefined") {
                ...
                (
                    ...
                    FROM hv_car_home_page_images hv_car_images
                    WHERE hv_car_images.horizon_id = " . $this->session->userdata('horizon_id') . "
                    AND hv_car_images.featured_iamge = 1
                    GROUP BY car_d_id
                ) AS hv_car_images
                ON
                ...
                LEFT OUTER JOIN hv_car_models ON hv_car_models.m_id = hv_car_details.m_id
                    WHERE hv_car_details.is_trash = 0 AND hv_car_details.horizon_id = " . $this->session->userdata('horizon_id') . "  
                ...
                hv_car_images.car_d_id = hv_car_details.car_d_id
                     LEFT OUTER JOIN hv_car_manufacturer ON hv_car_manufacturer.vm_id = hv_car_details.vm_id
                LEFT OUTER JOIN hv_car_models ON hv_car_models.m_id = hv_car_details.m_id 
                WHERE (VehicleNo LIKE CONCAT('%', ?, '%') OR vm_name LIKE CONCAT('%', ?, '%') OR model_name LIKE CONCAT('%', ?, '%') OR carname LIKE CONCAT('%', ?, '%') OR carprice LIKE CONCAT('%', ?, '%'))
                AND hv_car_details.is_trash = 0 AND hv_car_details.horizon_id = " . $this->session->userdata('horizon_id');

				$params = array("VehicleNo" => $search_term, "vm_name" => $search_term, "model_name" => $search_term, "carname" => $search_term, "carprice" => $search_term);
