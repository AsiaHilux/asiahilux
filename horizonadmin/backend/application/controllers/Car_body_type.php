...
class car_body_type extends CI_Controller
{
    private $limit = 50;
    private $this_page_id = "car_body_type_pg_id";
    ...
    public function index()
    {
        ...
        if ($this->check_permission("read", true)) {
            ...
            $this->load->view('car_body_type', $user_data);
        } else {
            $user_data['arr_user'] = $this->get_user_data($this->session->userdata('user_id'));
            $user_data['arr_current_school'] = $this->get_corrent_school($this->session->userdata('horizon_id'));
    ...
    public function load_new()
    {
        if ($this->check_permission("write", false)) {
            $this->load->view('car_body_type_form');
        } else {
    ...
    public function load_view()
    {
        if ($this->check_permission("read", false)) {
            $this->load->view('car_body_type_list');
        } else {
            http_response_code(403);
        }
    }
    ...
    public function load_edit()
    {
        if ($this->check_permission("write", false)) {
            ...
            if (count($result) > 0) {
                ...
                $this->load->view('car_body_type_form', $data);
            } else {
    ...
    public function create()
    {
        if ($this->check_permission("write", false)) {
            ...
            $bodytype_name = str_replace('"', "", $this->input->post('bodytype_name'));
            ...
            $car_bodytype_content = str_replace('"', "", $this->input->post('car_bodytype_content'));

            $data['horizon_id'] = $horizon_id;
            $data['bodytype_name'] = $bodytype_name;
            $data['active'] = $active;
            $data['Title'] = $Title;
            $data['Keywords'] = $Keywords;
            $data['Description'] = $Description;
            $data['car_bodytype_content'] = $car_bodytype_content;
            

            $res = $this->mdl_dal->insert_data("hv_car_bodytype", $data);
    ...
    public function update()
    {
        if ($this->check_permission("write", false)) {
            ...
            $setdata['bodytype_name'] = str_replace('"', "", $this->input->post('bodytype_name'));
            ...
            $setdata['car_bodytype_content'] = str_replace('"', "", $this->input->post('car_bodytype_content'));
            $setdata['active'] = str_replace('"', "", $this->input->post('active'));

           
            $wheredata['bodytype_id'] = str_replace('"', "", $this->input->post('bodytype_id'));

            $res = $this->mdl_dal->update_data('hv_car_bodytype', $setdata, $wheredata);
    ...
    public function delete()
    {
        if ($this->check_permission("delete", false)) {
            ...
            $horizon_id = $this->session->userdata('horizon_id');
            $bodytype_id= str_replace('"', "", $this->input->post('bodytype_id'));

            //check if this bdy type is used in Car details.
            $sql = "SELECT * FROM hv_car_details
            WHERE Bodytype = ? 
            AND horizon_id = ?";
            $params = array('Bodytype' => $bodytype_id, 'horizon_id' => $horizon_id);

            $records = $this->mdl_dal->select_data2($sql, $params);

            if (count($records) < 1) {
                $wheredata['bodytype_id'] = $bodytype_id;
                $wheredata['horizon_id'] = $horizon_id;

                $res = $this->mdl_dal->delete_data('hv_car_bodytype', $wheredata);
    ...
    function select()
    {
        if ($this->check_permission("read", false)) {
            ...
            {
                $sql = "SELECT * FROM hv_car_bodytype WHERE horizon_id = " . $this->session->userdata('horizon_id'). "  
                ORDER BY bodytype_id DESC ";
                ...
                $sql = "SELECT * FROM hv_car_bodytype 
                WHERE (bodytype_name LIKE CONCAT('%', ?, '%'))
                AND horizon_id = " . $this->session->userdata('horizon_id');
                $params = array("bodytype_name" => $search_term);
                $total_rows = $this->mdl_dal->select_num_rows($sql, $params);
            ...
            $config['base_url'] = base_url() . 'index.php/car_body_type/select';
    ...
    function select_active()
    {
        if (!$this->check_authentication()) {
            ...
            ///
            $sql = "SELECT * FROM hv_car_bodytype WHERE active = 1 AND hv_car_bodytype.horizon_id = " . $this->session->userdata('horizon_id');
    ...
    function select_by_id($id)
    {
        ...
        exit();
              ...
              $sql = "SELECT * FROM hv_car_bodytype WHERE bodytype_id= " . $id .
    ...
    public function upload_attachment_for_featured_Image()
    {
        ...
        if ($this->upload->do_upload($file)) 
        {
            ...
            $bodytype_id = str_replace('"', "", $this->input->post('car_featured_id_for_upload'));

           var_dump($bodytype_id);
